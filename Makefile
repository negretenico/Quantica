.PHONY: up down e2e-up e2e-down e2e-logs dev-ui run-server run-notify build test build-listener build-transformer build-bard build-risk build-trade build-notify build-server build-ui test-listener test-transformer test-bard test-risk test-trade test-notify test-server test-ui test-backtest test-shared tunnel info logs health
